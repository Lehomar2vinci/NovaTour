...
// TSV qui contient name + iso_n3 + iso_a2
...
async function loadCountryNames() {
  // Parse TSV -> map iso_n3 -> name
  const tsv = await fetchText(COUNTRY_TSV_URL, "country TSV");
  const rows = d3.tsvParse(tsv);

  const map = new Map();
  for (const r of rows) {
    // keys in TSV: name, iso_n3 (strings, sometimes with leading zeros)
...
function paintPins() {
  ...
  const pinsData = entries
    .map(([id, count]) => {
      const key = String(Number(id)); // normalize
  ...
  sel.merge(enter)
    ...
    .on("click", (event, d) => {
      ...
      showTooltip(event.clientX, event.clientY,
        `<div class="t">📍 ${escapeHtml(d.countryName)} — ${d.count} pin(s)</div>${lines}`
      );
    });
...
async function boot() {
  try {
    ...
    // 1) charge noms pays (pour tooltips)
    await loadCountryNames();

    // 2) charge topojson carte
